...
class BotData:
    def __init__(self, user_greeting_en, user_greeting_de, bot_greeting_en, bot_greeting_de, bot_ask_item_name_en, bot_ask_item_name_de,
        ...
        self.system_message = '''You are an accounting function calling AI model. You are provided with function signatures within <tools></tools> XML tags. You may call one or more functions to assist with the user query. Here are the available tools: <tools>\n{'type': 'function', 'function': {'name': 'accounting_invoice', 'description': 'Accounting processing with item input.', 'parameters': {'type': 'object', 'properties': {'item_name': {'type': 'string'}}, 'required': ['item_name']}}}\n</tools>\nFor each function call return a json object with function name and arguments within <tool_call></tool_call> XML tags as follows:\n<tool_call>\n{'arguments': <args-dict>, 'name': <function-name>}\n</tool_call>'''
    ...
    def _bot_function_quest(self, lang="en", all=False):
        ...
        if lang == "en":
            if not all:
                ...
                assistant_message = f"<tool_call>\n{{'arguments': '{{'item_name': '{item}'}}', 'name': 'accounting_invoice'}}\n</tool_call>"
...
if __name__ == "__main__":
    ...
    file_name = "prompt_data_hermes.xlsx"
    dataset = []
